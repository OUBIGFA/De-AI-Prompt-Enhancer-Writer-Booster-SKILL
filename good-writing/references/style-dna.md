...
**关键规律：一段之内编织多个论点。** 比如《从鲁迅弃医从文想到的》第五段（「什么是幸灾乐祸？」开始），在一个段落里完成了：定义→分类→第一类批判→第二类批判→类比反问。如果用AI的方式写，这会被拆成5个段落。
...
→ 一个段落，约350字。内含：定义、两类人的批判、类比反问。全部用逗号和分号连接成绵密的语流。
...
不是"短句+短句+短句"的子弹式打击，而是"长句长句长句——偶尔一个短句顿挫"。
...
- 「我听到这句话的第一反应不是愤怒，而是困惑」——戏剧化反应，像电影旁白
...
原作者当然会用冒号，但几乎只用在**功能性场景**：

1) **引出分项/编号**（后面真的有 １）２） 这类结构）
2) **引出术语定义**（枯燥、精确，不拿冒号做气势）
3) **引出原句/引文**（后面紧跟引号或引文块）
...
→ 冒号之后马上进入严格分项（１）２）３）４）），冒号承担的是结构功能。

**例证2：引出定义**（《问世间性是何物》）

> 在生物学上，性有一个非常枯燥的定义：基因的重组，也就是来自两个以上的个体的遗传物质的融合。

→ 冒号后不是“抒情”或“强调”，而是定义本体。

**例证3：引出引文/原句**（《从鲁迅弃医从文想到的》）
...
- 把冒号当“结构提示器”：`原因是：` `关键在于：` `一句话总结：`
- 把冒号当“气势加成”：`因为数字摆在那里：` `事实摆在那里：`
...
**关键规律：比喻是调味料，不是主菜。** 原作者的每个比喻都在一句内完成，从不展开成详细的类比场景。
