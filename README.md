...
---
2026 默认行为（`good-writing` / `de-AI-writing`）：
- 默认输出纯正文成品（非审阅任务不输出 `##` 小标题、列表、加粗、评分说明）
- 正文禁用“而是”骨架（用户逐字引文或不可改动片段除外）
- 最终成品正文克制冒号（`：`/`:` 合计每 500 字 ≤ 1，避免路标式冒号结构）
- 改写任务默认反缩水（用户未明确要求精简时，改写后字数不低于原文）
- `de-AI-writing` 在 `good-writing` 底座上额外约束第一人称，默认“我/我们”合计不超过 2 次
