...
- [ ] **段落节奏与密度**：1500-3000字正文是否在 `8-12` 段（信息综述类可放松到 `10-14`）？是否厚段 `>=4` 且承担主论证？是否避免连续 `3` 段 `<90` 字和连续 `2` 段一句成段？薄段/一句成段是否都承担了明确功能而非路标切段？
...
- [ ] **比喻密度**：每 1000 字比喻是否 ≤ 2 个？是否有展开式类比或对价式比喻？是否使用了 AI 高频隐喻词（噪音/背景噪音、信号、底色、光谱、滤镜、解药、土壤、基因、拼图、镜像、路径）的比喻义？
...
1. **禁止碎段失控**：不准出现连续 `3` 段 `<90` 字；不准连续 `2` 段一句成段
...
9. **禁止标题党独立短句连发**：允许少量一句成段作为顿挫（1500-3000字通常 `1-3` 次），但不准连续出现，也不准写成字幕式切段
10. **禁止bullet point和列表**：所有内容必须散文化表达
11. **禁止比喻过载**：每 1000 字比喻不超过 2 个，禁止对价式比喻和展开式类比；另禁以下 AI 高频隐喻词的**比喻义**用法（字面义不受限）：「噪音/背景噪音」「信号」「底色」「光谱」「滤镜」「解药」「土壤」「基因」（非生物学语境）「拼图」「镜像」「路径」——这些词用作比喻时是 LLM 的统计高频输出，原作者从未使用
